...
static gorev_t gorevHeap[5];
static gorevListeOgesi_t calisanListesi[5];



static gorevListeOgesi_t cikanListesi[(unsigned char)(5 / 3)];
...
static void listeyeEkle(gorevListeOgesi_t liste[], char *sayim, char max, gorevListeOgesi_t gkb) {
...
static void listedenSil(gorevListeOgesi_t liste[], char *sayim, const char konum) {
    const char cSayim = *sayim;
...
gorev_t *gorevOlustur(is_t is) {
 ...
 gkb->is = is;
 ( (gorev_t*) gkb )->sn = 0;
    listeyeEkle(calisanListesi, &calisanSayimi, 5, gkb);
 return gkb;
}
...
void gorevciyiBaslat() {
 while(1) {
  for(char s = 0; s < gorevSayimi; s++) {
   char donus = calisanListesi[s]->is(calisanListesi[s]);
            if(donus == 1) {
                ...
                listeyeEkle(cikanListesi, &cikanSayimi, (unsigned char)(5 / 3), glo);
            }
  }
 }
}
