...
# 36 "../../../../../kaynak/portlar/mplabx/xc8/pic18/portmacro.h"
...
# 37 "../../../../../kaynak/portlar/mplabx/xc8/pic18/portmacro.h" 2
...
pgkb grvOlustur(is_t is) {
 ...
 if(gorevSayimi >= 8) return ((void*)0);
...
void grvBaslat(const unsigned char kimlik) {
 pgkb gkb = grvKimlikIleGorevBlogunuAl(kimlik);
 if(gkb == ((void*)0)) return;
 ( (gorev_t*) gkb )->sn = 0;
 gkb->durum = 0;
}
...
void grvGorevciyiBaslat() {
 while(1) {
  for(unsigned char s = 0; s < gorevSayimi; s++) {
   ...
   pgkb gkb = &gorevHavuzu[s];

   if(gkb == ((void*)0)) continue;
...
unsigned int grvTikSayiminiAl() {
 ...
 intconYedek = INTCON; INTCONbits.GIEH = 0;;
 sayim = tikSayimi;

 if(intconYedek & 0x80){ INTCONbits.GIEH = 1;; };
 return sayim;
}
