...
static gorev_t gorevHeap[6];
static gorevListeOgesi_t calisanListesi[6];
# 62 "../../../../../kaynak/gorev.c"
static gorevListeOgesi_t cikanListesi[2];
...
gorev_t *gorevOlustur(is_t is) {
 ...
 gkb->is = is;
 ( (gorev_t*) gkb )->sn = 0;
    listeyeEkle(calisanListesi, &calisanSayimi, 6, gkb);
 return gkb;
}
  ...
  for(char s = 0; s < gorevSayimi; s++) {
   char donus = calisanListesi[s]->is(calisanListesi[s]);
            if(donus == 1) {
                ...
                listeyeEkle(cikanListesi, &cikanSayimi, 2, glo);
            }
  }
 }
}
