...
static gorev_t gorevHeap[8];
static gorevListeOgesi_t calisanListesi[8];
# 62 "../../../../../kaynak/gorev.c"
static gorevListeOgesi_t cikanListesi[3];
...
gorev_t *gorevOlustur(is_t is) {
 ...
 gkb->is = is;
 ( (gorev_t*) gkb )->sn = 0;
    listeyeEkle(calisanListesi, &calisanSayimi, 8, gkb);
 return gkb;
}
  ...
  for(char s = 0; s < gorevSayimi; s++) {
   char donus = calisanListesi[s]->is(calisanListesi[s]);
            if(donus == 1) {
                ...
                listeyeEkle(cikanListesi, &cikanSayimi, 3, glo);
            }
  }
 }
}
